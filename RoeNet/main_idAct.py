...
def test():
    f_neur.load_state_dict(torch.load("model_idAct.pt", map_location=lambda storage, location: storage))
    f_neur.to(device)
    f_neur.eval()
    ...
    check_dim = 0
    with torch.no_grad():
        with open('nontrivialid.dat','w') as f:
...
def train():
    ...
    for i in range(n_epoch):
        ...
        if lowest_test_loss > test_loss / test_sample:
            torch.save(f_neur.state_dict(), "model_idAct.pt")
